...
export const localDataLoader: LocalDataLoader = {
  abilitiesByShowdownId: () => {
    if (!_dataCache.abilitiesByShowdownId) {
      _dataCache.abilitiesByShowdownId = new Map<string, Ability>(
        readFileAsJson<Ability[]>(getDataPath("abilities.json")).map(
          (record) => [record.psName, record]
        )
      );
    }
  ...
  movesByShowdownId: () => {
    if (!_dataCache.movesByShowdownId) {
      _dataCache.movesByShowdownId = new Map<string, Move>(
        readFileAsJson<Move[]>(getDataPath("moves.json")).map((record) => [
          record.psName,
          record,
        ])
      );
  ...
  itemsByShowdownId: () => {
    if (!_dataCache.itemsByShowdownId) {
      _dataCache.itemsByShowdownId = new Map<string, Item>(
        readFileAsJson<Item[]>(getDataPath("items.json")).map((record) => [
          record.psName,
          record,
        ])
      );
  ...
  boxPresets: () => {
    if (!_dataCache.boxPresets) {
      const boxPresetsById = mergeEntityIndex<BoxPreset>(
        "boxpresets-index.json",
        "gameSet"
      );
  ...
  ribbons: () => {
    if (!_dataCache.ribbons) {
      _dataCache.ribbons = readFileAsJson<Ribbon[]>(
        getDataPath("ribbons.json")
      );
    }
  ...
  pokemon: () => {
    if (!_dataCache.pokemon) {
      _dataCache.pokemon = mergeEntityIndex<Pokemon>(
        "pokemon-index.json",
        "region"
      );
    }
  ...
  pokedexes: () => {
    if (!_dataCache.pokedexes) {
      _dataCache.pokedexes = mergeEntityIndex<Pokedex>(
        "pokedexes-index.json",
        "region"
      );
    }
